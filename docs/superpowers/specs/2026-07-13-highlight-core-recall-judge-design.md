...
**Unreliable word timings (music/crosstalk):** such regions are opaque nodes with REAL segment-level edges (Whisper segment boundaries) - only their word timings are untrustworthy. An opaque payoff or end therefore ships at `boundaryConfidence: "segment"` (the cut rides the segment edge, V1-grade accuracy) instead of dropping; code still prefers walking the end back to a word-bearing node at or after the payoff. A start immediately AFTER an opaque node counts as a clean cold open (a music/silence break is a strong semantic boundary even though leadingStrength inherits the opaque neighbor's 0.2). Capitalization is a veto signal: pause-minted 0.8 boundaries and post-opaque starts additionally require the node text to NOT open with a lowercase letter (hesitation pauses fake boundaries mid-sentence; Whisper capitalizes real sentence starts); terminal-punctuation boundaries (1.0) are trusted as-is. The END mirrors this via isCleanEnd: a clip may not end on a weak boundary whose continuation opens lowercase - repair trims BACK to the latest clean end at or after the payoff (never swallowing a foreign sentence), completes FORWARD within 3s only when the payoff's own sentence is still open, else drops with no_clean_end. Drops remain only when the end would land before the payoff. (The earlier anchor-text fallback is removed - see §12.)
...
| Cut mid-thought | Minimized: clean-start check (strong boundary + capitalization veto, walk-back <= 6s, else drop) + clean-END check (isCleanEnd with back-trim/forward-complete repair, else no_clean_end drop) + ¶ markers + critic boundary rules |
