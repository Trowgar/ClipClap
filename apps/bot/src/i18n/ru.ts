...
const ruFailure: Record<JobErrorCode, string> = {
  UNSUPPORTED_INPUT:
    "Я не смог прочитать этот файл как видео. Возможно, он повреждён, загрузился не полностью или в нём нет видеодорожки. Проверь, что исходный файл воспроизводится до конца, и пришли исправный видеофайл. Минуты не списаны.",
...
const ru: Dict = {
  ...
  deliveryGivenUp: (url, clips) => {
    if (clips === 0) {
      return `Не удалось доставить результат по этому видео в этот чат - больше не пытаюсь. На моей стороне ничего не потеряно. Не присылай это видео заново, пока не получишь ответ: повторная обработка спишет минуты второй раз. Напиши в поддержку из меню помощи - расскажу, чем всё закончилось.`;
    }
    ...
    return `${ready}, но отправить ${them} в этот чат не получилось - больше не пытаюсь. Ничего не потеряно: напиши в поддержку из меню помощи, и я передам ${them} тебе. Не присылай это видео заново: клипы уже есть, а повторная обработка спишет минуты второй раз.`;
  },
  doneNoClips: (reason) =>
  ...
  freeExhausted: (remainingMinutes, lifetimeMinutes, planMinutes, planPriceEur, offer) =>
    `Бесплатных минут на это не хватит - осталось ${remainingMinutes} из ${lifetimeMinutes}. Всё, что я уже успел нарезать, остаётся у тебя.\n\n${offer ?? `Чтобы продолжить: Starter - €${planPriceEur} в неделю за ${planMinutes} минут видео, исходники до 3 часов и 20 клипов, которые хранятся 7 дней.`}`,
  ...
  sourceTooShort:
    "Это видео короче минуты. Я вырезаю клипы из длинного разговора - стримов, подкастов, интервью, лекций - а тут пока нечего резать. Пришли что-то хотя бы на минуту; лучше всего получается от десяти минут.",
  shortSourceNotice:
    "Предупреждаю: это видео короче пяти минут. Из коротких исходников обычно выходит 0-2 клипа - лучше всего я работаю с десятью минутами и длиннее.",
  duplicateActive:
    "Я уже работаю ровно над этим видео - напишу, как только будет готово. Второй раз присылать не нужно.",
  duplicateDone: (clipCount) =>
    `Это видео ты мне уже присылал - вот ${clipCount === 1 ? "клип, который" : `${clipCount} клипов, которые`} я из него сделал, минуты не потрачены.`,
  ...
  plansText:
    ...
    "Исходники до 180 минут; остатка минут должно хватать на всё видео. Starter на неделю включает 75 минут: для видео длиннее 75 минут выбери месячный тариф (270 минут) или пришли фрагмент покороче.\n\n" +
    "Выбери план ниже 👇",
  ...
  urlAccessFailed:
    "Не удалось получить видео по этой ссылке. Пожалуйста, загрузи видеофайл сюда напрямую.",
  urlYouTubeUnavailable:
    "YouTube блокирует доступ к видео. Пожалуйста, загрузи видеофайл сюда напрямую.",
  purchaseResubmit: "После подтверждения оплаты пришли видео ещё раз, чтобы начать обработку.",
