...
export async function runQualityLane(input: QualityLaneInput): Promise<QualityLaneResult> {
  ...
  if (cfg.safeEndAuditMode === "shadow") {
    ...
    const normalAudit = jsonSafeEndAuditTelemetry(
      ...
      afterPostBoundaryHookGate,
      languageIso,
      input.safeEndAuditTelemetryTestHook
    );
    safeEndAuditTelemetry = {
      ...normalAudit,
    };
  ...
  for (const candidate of input.candidates) {
    ...
    if (!verdict.keep) {
      terminal.set(candidate.id, "critic_rejected");
      continue;
    }
    if (shippedIds.has(candidate.id)) {
      terminal.set(candidate.id, "shipped");
      continue;
    }
    const drop = droppedById.get(candidate.id);
    if (drop?.stage === "gate") terminal.set(candidate.id, "evidence_rejected");
    else if (drop?.stage === "snap" || drop?.stage === "long_clip_policy") terminal.set(candidate.id, "snap_rejected");
