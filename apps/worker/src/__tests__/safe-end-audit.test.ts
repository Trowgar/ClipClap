...
describe("safe-end audit primitives", () => {
  it("detects only an immediate word-bearing zero-tail handoff", () => {
    expect(zeroTailHandoff(clip("at-edge", { finalEndNode: 0 }), nodes([[8, 10, true], [10.05, 12, true]]))).toBe(true);
    expect(zeroTailHandoff(clip("over-edge", { finalEndNode: 0 }), nodes([[8, 10, true], [10.051, 12, true]]))).toBe(false);
  ...
  it("caps normal detail by closed severity then candidate id", () => {
  ...
  it("caps rescue detail by closed severity and retains the selected record", () => {
  ...
  it("keeps at most twenty detailed records even when a caller requests more", () => {
    const records = Array.from({ length: 22 }, (_, index) => normal(`safe-${index.toString().padStart(2, "0")}`, "safe"));
    const result = capSafeEndNormalRecords(records, 99);
  ...
  it("reconciles actual geometry membership without mutating clips or records", () => {
    ...
    const reconciled = reconcileSafeEndNormalRecords(
      records,
      [clip("finalizer"), clip("soft"), source],
      [clip("soft"), source],
      [shipped],
    );
