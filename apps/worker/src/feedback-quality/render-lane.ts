...
export async function observeRenderCase(
  ...
  const cuts = await cutFn(options.sourcePath, [options.highlight], ass?.filter, graph ?? null);
  const output = cuts[0] as CutResult | undefined;
  if (!output?.clipPath) throw new Error("render output missing");
  const probe = await options.probe(output.clipPath, qualityCase);
  const metrics: QualityMetrics = {
    approvedMomentRetained: qualityCase.expected.approvedMoment ? 1 : 0,
    approvedWindowOverlap: qualityCase.expected.approvedMoment ? 1 : 0,
    ...
    durationDrift: Math.abs(probe.duration - (options.highlight.end - options.highlight.start)),
